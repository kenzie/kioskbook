...
2. Run installer: `sudo ./install.sh [display_token] [tailscale_key]`
...
vim configs/lightdm/lightdm.conf
# Note: display.conf is generated by 50-app.sh module

# Then update via module:
sudo bash modules/70-services.sh  # Direct test (for monitor changes)
sudo bash modules/30-display.sh   # Direct test (for lightdm changes)
# OR
...
sudo bash modules/70-services.sh
